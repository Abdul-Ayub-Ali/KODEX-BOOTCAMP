const UserCard = ({ card, handleDelete, setUpdate, setToggle }) => {
  return (
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-gradient-to-tr from-indigo-600 to-purple-500 rounded-2xl flex items-center justify-center shadow-lg transform group-hover:rotate-6 transition-transform">
              <span className="text-2xl font-bold text-white">{card.fullName.split(" ")
                .map(word => word[0])     
                .join("") }</span>
            </div>
            <div>
        ...
        <div className="mt-6 flex items-center justify-between pt-4 border-t border-slate-100">
          <div className="flex gap-2">
            ...
            <button onClick={() => handleDelete(card.id)} className="px-4 py-2 text-xs font-bold bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors">
              Delete
            </button>
          </div>
